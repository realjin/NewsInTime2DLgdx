package info.realjin.newsintime2d.service.retriever;
...
import info.realjin.newsintime2d.domain.RssFeed;
import info.realjin.newsintime2d.domain.RssItem;
...
public class RetrieverServiceThread implements Runnable {
	RetrieverService nrs;
	...
	public RetrieverServiceThread(RetrieverService nrs, NewsListWrapper nl,
			Collection coll) {
		this.nrs = nrs;
		this.nl = nl;
		this.coll = coll;
		...
		for (RssFeed rf : rfList) {
			for (Object o : rf.getAllItems()) {
				...
				if (ri.getPubDate() == null) {
					...
					pubDate = RetrieverService.parsePubDate(ri.getPubDate());
				}
